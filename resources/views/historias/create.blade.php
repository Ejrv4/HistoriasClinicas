...
<div class="container-fluid p-0">
    ...
    <form action="{{ route('historias.store') }}" method="POST" id="formAtencionMedica" autocomplete="off">
        ...
        <div class="tab-content" id="hcTabsContent">
            ...
            {{-- PESTAÑA ANTECEDENTES (PRECARGADOS SI YA EXISTÍAN) --}}
            <div class="tab-pane fade show active" id="pestana-antecedentes" role="tabpanel">
                <div class="card border-0 shadow-sm p-4 rounded-4 bg-body-card">
                    <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">
                        <h5 class="fw-bold text-dark-mode-title mb-0 d-flex align-items-center"><i class="bi bi-file-earmark-medical-fill text-primary me-2 fs-4"></i>Antecedentes Generales</h5>
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <button type="button" onclick="guardarAntecedentesManual(event)" class="btn btn-success fw-bold shadow-sm rounded-3 border-0 px-3 py-2" style="background-color: #10b981; font-size: 0.85rem;">
                                <i class="bi bi-save2-fill me-2"></i>GUARDAR ANTECEDENTES
                            </button>
                        </div>
                    </div>
                    ...
                    <div id="formAntecedentesContenedor">
                        <div class="row g-4">
                            <div class="col-md-6">
                                ...
                                <textarea name="Medico" class="form-control rounded-3" rows="3" placeholder="Describir antecedentes médicos...">{{ $antecedentes->where('tipo', 'Médico')->first()->descripcion ?? '' }}</textarea>
                            </div>
                            <div class="col-md-6">
                                ...
                                <textarea name="Quirúrgico" class="form-control rounded-3" rows="3" placeholder="Describir intervenciones quirúrgicas...">{{ $antecedentes->where('tipo', 'Quirúrgico')->first()->descripcion ?? '' }}</textarea>
                            </div>
                            <div class="col-md-6">
                                ...
                                <textarea name="Alergia" class="form-control border-danger-subtle rounded-3" rows="3" placeholder="Especificar alergias o intolerancias...">{{ $antecedentes->where('tipo', 'Alergia')->first()->descripcion ?? '' }}</textarea>
                            </div>
                            <div class="col-md-6">
                                ...
                                <textarea name="Medicación" class="form-control border-success-subtle rounded-3" rows="3" placeholder="Medicamentos de uso diario...">{{ $antecedentes->where('tipo', 'Medicación')->first()->descripcion ?? '' }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="card border-0 shadow-sm rounded-4 overflow-hidden border bg-body-card">
                            ...
                            <div class="card-body p-0">
                                <ul class="list-group list-group-flush small font-monospace" id="lista-referencia">
                                    ...
                                    @empty
                                        <li class="list-group-item bg-transparent text-muted fst-italic p-3 text-center small">Sin registros previos en base de datos.</li>
                                    @endforelse
                                </ul>
                            </div>
                        </div>
                    ...
                    <div class="col-md-9">
                        <div class="card border-0 shadow-sm p-4 rounded-4 bg-body-card">
                            <div class="mb-3">
                                ...
                                <textarea name="anamnesis" class="form-control rounded-3" rows="4" placeholder="Escribir motivo de consulta y síntomas narrados por el paciente..."></textarea>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="examen_fisico" class="form-control rounded-3" rows="4" placeholder="Registrar funciones vitales y hallazgos exploratorios..."></textarea>
                            </div>
                            ...
                            <div class="mb-0">
                                ...
                                <textarea name="plan" class="form-control rounded-3 textarea-expandable" rows="4" placeholder="Indicaciones médicas generales adicionales..."></textarea>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="tab-pane fade" id="pestana-receta" role="tabpanel">
                <div class="card border-0 shadow-sm p-4 rounded-4 bg-body-card">
                    ...
                    <div class="text-end border-top pt-4">
                        <button type="submit" class="btn btn-primary btn-lg px-5 shadow-lg fw-bold rounded-3 border-0" style="background: linear-gradient(135deg, #4e73df 0%, #224abe 100%);">
                            <i class="bi bi-check-all me-1.5 fs-5"></i> FINALIZAR ATENCIÓN MÉDICA
                        </button>
                    </div>
                </div>
            </div>
    ...
    {{-- ACORDEÓN DE HISTORIAL RE-ESTILIZADO --}}
    <div class="mt-5 mb-5">
...
{{-- Los scripts originales enriquecidos permanecen aquí debajo --}}
<script>
    ...
    function configurarNavegacionTecladoParaDropdowns() {
        document.addEventListener('keydown', function(e) {
            ...
            if (e.key === 'ArrowDown') {
                ...
                // Si presionó Tab o Enter, autoseleccionar el item resaltado o el primer visible
    ...
    atencionForm.addEventListener('submit', function(e) {
        ...
        if (errorDiag) {
            ...
            // Cambiar a la pestaña de Historia Clínica si no está visible
